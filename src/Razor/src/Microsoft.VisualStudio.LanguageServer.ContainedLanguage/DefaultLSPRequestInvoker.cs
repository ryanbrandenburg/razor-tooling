...
namespace Microsoft.VisualStudio.LanguageServer.ContainedLanguage
{
    ...
    internal class DefaultLSPRequestInvoker : LSPRequestInvoker
    {
        ...
        public override Task<ReinvokeResponse2<TOut>> ReinvokeRequestOnServerAsync<TIn, TOut>(ITextBuffer textBuffer, string method, string languageServerName, TIn parameters, CancellationToken cancellationToken)
        {
            var capabilitiesFilter = _fallbackCapabilitiesFilterResolver.Resolve(method);
        ...
        public override async Task<ReinvokeResponse2<TOut>> ReinvokeRequestOnServerAsync<TIn, TOut>(
            ...
            CancellationToken cancellationToken)
        {
            ...
            var result = resultToken != null ? new ReinvokeResponse2<TOut>(languageClientName, resultToken.ToObject<TOut>(_serializer)) : default;
            return result;
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public override Task<IEnumerable<ReinvokeResponse2<TOut>>> ReinvokeRequestOnMultipleServersAsync<TIn, TOut>(ITextBuffer textBuffer, string method, TIn parameters, CancellationToken cancellationToken)
        {
            var capabilitiesFilter = _fallbackCapabilitiesFilterResolver.Resolve(method);
            return ReinvokeRequestOnMultipleServersAsync<TIn, TOut>(textBuffer, method, capabilitiesFilter, parameters, cancellationToken);
        }

        public override async Task<IEnumerable<ReinvokeResponse2<TOut>>> ReinvokeRequestOnMultipleServersAsync<TIn, TOut>(
            ITextBuffer textBuffer,
            string method,
            Func<JToken, bool> capabilitiesFilter,
            TIn parameters,
            CancellationToken cancellationToken)
        {
            ...
            var results = clientAndResultTokenPairs.Select((clientAndResultToken) => clientAndResultToken.Item2 != null ? new ReinvokeResponse2<TOut>(clientAndResultToken.LangaugeClientName, clientAndResultToken.Response.ToObject<TOut>(_serializer)) : default);
            return results;
        }
    }
}
