...
internal class RazorDocumentOnTypeFormattingEndpoint : IVSDocumentOnTypeFormattingEndpoint
{
    ...
    public async Task<TextEdit[]?> HandleRequestAsync(DocumentOnTypeFormattingParamsBridge request, RazorRequestContext requestContext, CancellationToken cancellationToken)
    {
        ...
        if (!request.Position.TryGetAbsoluteIndex(sourceText, logger: null, out var hostDocumentIndex))
        {
            return null;
        }
