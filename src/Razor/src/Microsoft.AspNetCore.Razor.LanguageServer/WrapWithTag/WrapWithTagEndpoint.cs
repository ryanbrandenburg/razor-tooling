...
namespace Microsoft.AspNetCore.Razor.LanguageServer.WrapWithTag
{
    internal class WrapWithTagEndpoint : IWrapWithTagEndpoint
    {
        ...
        public async Task<WrapWithTagResponse?> HandleRequestAsync(WrapWithTagParamsBridge request, RazorRequestContext requestContext, CancellationToken cancellationToken)
        {
            var documentContext = requestContext.DocumentContext;
            if (documentContext is null)
            {
                requestContext.Logger.LogWarning("Failed to find document {request.TextDocument.Uri}.", request.TextDocument.Uri);
                return null;
            }
